"""
27_train_rs_multitask.py
========================
Two-Tower con mejoras conservadoras sobre el baseline (23_):

1. Cutoff pre-2017 para Two-Tower (era pre-2016) → +3,600 items con señal colaborativa
2. Multi-task: head auxiliar predice log(review_count) → BCE + λ_pop * MSE
3. BCE loss (mantenemos lo que funciona — InfoNCE con temp=0.1 fue contraproducente)
4. User content features (versión conservadora: CS_DROP también en user tower)

Volvemos a BCE porque la InfoNCE con temperatura baja overfitteó el espacio colaborativo.
"""
...
print("=" * 70)
print("27_train_rs_multitask.py — BCE + Multi-task + cutoff 2017")
print("=" * 70)
...
for epoch in range(1, EPOCHS + 1):
    ...
    for u_batch, i_batch in train_loader:
        ...
        with torch.amp.autocast(device_type=DEVICE.type, enabled=(DEVICE.type=="cuda")):
            logits, item_emb_pos = model(users_all[:len(u_batch)],
                                         items_all[:len(u_batch)],
                                         cont_all[:len(u_batch)])
            # BCE sobre todos (pos + neg)
            logits_all, _ = model(users_all, items_all, cont_all)
            loss_bce = criterion(logits_all, labels)

            # Multi-task: popularidad solo de positivos
...
np.save(os.path.join(DATA, "item_embeddings_rs_v2_multitask.npy"),    item_emb_std)
np.save(os.path.join(DATA, "item_embeddings_rs_v2_multitask_cs.npy"), item_emb_cs)
...
def eval_catboost(item_emb, label, n_trials=60):
    ...
    save_result(model_id=f"27_{label}_cat",
                model_name=f"MultiTask TT ({label}) + CatBoost",
                features=f"RS multitask ({label}) (64d) + meta (6d)",
                embeddings="v2_global",
                metrics=metrics,
                notes=f"v2 cutoff2017TT BCE+multitask λ={LAMBDA_POP}: "
...
# ── 11. Resumen ────────────────────────────────────────────────────────────────
print("\n" + "=" * 70)
print("RESUMEN — 27_xx MultiTask TT (BCE + cutoff2017 + multi-task)")
print("=" * 70)
