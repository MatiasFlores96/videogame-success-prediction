"""
30_train_rs_cutoff2017.py
==========================
BCE puro + cutoff 2017, SIN multi-task head.

Hipotesis: el pop_head en 29_ introdujo leakage indirecto.
  - y_full incluye interacciones post-2017 (el "futuro")
  - El pop_head entrena item embeddings pre-2017 a predecir y_full (total)
  - CatBoost downstream ve embeddings que encodifican y_full => TSCV=0.94 pero temporal=-0.013
  - Para items post-2017 (test) el pop_head nunca corrió => embeddings sin señal

FIX: remover pop_head (LAMBDA_POP=0), queda BCE puro + cutoff 2017.
Aislamos si el cambio de cutoff pre-2016->pre-2017 SOLO mejora sobre el baseline.

Mejoras que SI quedan de 29_:
  + Split aleatorio (train_test_split random_state=42)
  + Single forward pass por batch
...
PATIENCE   = 4
BATCH      = 8192
LAMBDA_POP = 0.0   # SIN multi-task — aislar el efecto del cutoff 2017
WARMUP_EP  = 2

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

print("=" * 70)
print("30_train_rs_cutoff2017.py — BCE puro + cutoff 2017, sin multi-task (test leakage fix)")
print("=" * 70)
...
for epoch in range(1, EPOCHS + 1):
    ...
    for u_batch, i_batch in train_loader:
        ...
        with torch.amp.autocast(device_type=DEVICE.type, enabled=(DEVICE.type=="cuda")):
            # BCE puro — sin multi-task
            logits_all, _ = model(users_all, items_all, cont_all)
            loss_bce = criterion(logits_all, labels)
            loss = loss_bce
            loss_pop = torch.tensor(0.0)
...
np.save(os.path.join(DATA, "item_embeddings_rs_v2_cutoff2017.npy"),    item_emb_std)
np.save(os.path.join(DATA, "item_embeddings_rs_v2_cutoff2017_cs.npy"), item_emb_cs)
...
def eval_catboost(item_emb, label, n_trials=60):
    ...
    save_result(model_id=f"30_{label}_cat",
                model_name=f"BCE cutoff2017 TT ({label}) + CatBoost",
                features=f"RS cutoff2017 ({label}) (64d) + meta (6d)",
                embeddings="v2_global",
                metrics=metrics,
                notes=f"v2 cutoff2017TT BCE puro (no multitask): "
...
# ── 11. Resumen ────────────────────────────────────────────────────────────────
print("\n" + "=" * 70)
print("RESUMEN — 30_xx BCE puro + cutoff2017 (sin multi-task, fix-split, grad_clip)")
print("=" * 70)
