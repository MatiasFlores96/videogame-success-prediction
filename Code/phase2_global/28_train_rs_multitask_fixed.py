"""
28_train_rs_multitask_fixed.py
==============================
Fix del 27_train_rs_multitask.py:

BUG ENCONTRADO EN 27_: double forward pass por batch.
  - model() se llamaba DOS veces: una para positivos (item_emb_pos) y otra para pos+neg (BCE).
  - Al hacer backward(), todos los parametros recibían gradientes DOBLES.
  - Embeddings de usuarios con LR efectivo 2x -> overfitting inmediato (val sube desde epoch 2).

FIX: una sola llamada a model() para pos+neg y extraer item_emb_pos del primer bloque.
     + gradient clipping (norm=1.0) para mayor estabilidad.

Resto igual a 27_:
  - Cutoff pre-2017 (era pre-2016)
  - Multi-task: BCE + lambda_pop * MSE(log review_count)
  - User tower: ID-only (sin cambios)
"""
...
print("=" * 70)
print("28_train_rs_multitask_fixed.py — BCE + Multi-task + cutoff 2017 (fix double-fwd)")
print("=" * 70)
...
np.save(os.path.join(DATA, "item_embeddings_rs_v2_multitask_fixed.npy"),    item_emb_std)
np.save(os.path.join(DATA, "item_embeddings_rs_v2_multitask_fixed_cs.npy"), item_emb_cs)
...
def eval_catboost(item_emb, label, n_trials=60):
    ...
    save_result(model_id=f"28_{label}_cat",
                model_name=f"MultiTask TT fixed ({label}) + CatBoost",
                features=f"RS multitask fixed ({label}) (64d) + meta (6d)",
                embeddings="v2_global",
                metrics=metrics,
                notes=f"v2 cutoff2017TT BCE+multitask fixed λ={LAMBDA_POP}: "
...
# ── 11. Resumen ────────────────────────────────────────────────────────────────
print("\n" + "=" * 70)
print("RESUMEN — 28_xx MultiTask TT FIXED (BCE + cutoff2017 + multi-task + grad_clip)")
print("=" * 70)
